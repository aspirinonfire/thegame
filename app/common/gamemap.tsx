...
import { LicensePlateSpot } from './gameCore/gameModels';
...
export default function GameMap(mapArguments: ActiveGameData | HistoricalGamesData) {
  ...
  function getWeightClass(state: string, isSvg: boolean = true): string {
  ...
  function isChecked(province: string) {
    if (mapArguments.argType == "activeGame") {
      return !!mapArguments.plateSpots[province]?.dateSpotted
    } else {
      return !!mapArguments.spotsByStateLookup[province]
    }
  }
  ...
  function renderCanadaBadge(fullProvinceName: string, abbr: string) {
    ...
    if (mapArguments.argType == "activeGame") {
      ...
      additionalInfo = mapArguments.spotsByStateLookup[`ca_${abbr}`] ?? 0;
      isActiveGame = false;
      spottedProvinceClasses = "text-gray-100 bg-orange-800 hover:bg-orange-900";
    }

    const provinceIsChecked = isChecked(`ca_${abbr}`);
