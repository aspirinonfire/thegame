...
interface HistoricalGamesData {
  argType: "historicData"
...
const GameMap = (mapArguments: ActiveGameData | HistoricalGamesData) => {
  if (!mapArguments) {
    return (<p className="text-white text-lg animate-pulse">awaiting map data...</p>);
  }
  ...
  function getWeightClass(lkpKey: string, isSvg: boolean = true): string {
    if (mapArguments.argType == "activeGame") {
      ...
      const spotRate = Math.round((mapArguments?.spotsByStateLookup[lkpKey] ?? 0) / (mapArguments?.totalNumberOfGames ?? 1) * 100);
  ...
  return (
    <div className="flex flex-col gap-10 grow">
      ...
      <div className="flex flex-row justify-center flex-wrap gap-2 mt-3">
        ...
        { renderCanadaBadge("New Brunswick", "NB") }
        { renderCanadaBadge("Newfoundland and Labrador", "NL") }
        { renderCanadaBadge("Northwest Territories", "NT") }
        { renderCanadaBadge("Nova Scotia", "NS") }
        { renderCanadaBadge("Nunavut", "NU") }
        { renderCanadaBadge("Ontario", "ON") }
        { renderCanadaBadge("Prince Edward Island", "PE") }
        { renderCanadaBadge("Quebec", "QC") }
        { renderCanadaBadge("Saskatchewan", "SK") }
        { renderCanadaBadge("Yukon", "YT") }
      </div>
    </div>
  )
}
