  ...
  ILogger<DomainMessagesProcessor> logger)
{
  public async Task ListenAndProcessQueueEvents(CancellationToken cancellationToken)
  {
    ...
    try
    {
      ...
      logger.LogInformation("Message queue got closed. Message processing is now complete.");
    }
    catch (OperationCanceledException)
    {
      logger.LogInformation("Processor received cancellation. Message processing is now complete.");
    }
  }
}